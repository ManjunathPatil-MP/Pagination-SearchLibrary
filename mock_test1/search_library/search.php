        ...
        {   if($string_ids!=0)
            {
               ...
               {
                    $value_id="".$string_ids."";
                    //array_push($append_id, "mock_test_tbl".$key.".".$key." IN (".$value_id.")");
					array_push($append_id, "mock_test_tbl.".$key." IN (".$value_id.")");// Changes added by Manjunath
               }
               
            }
            if($value!='')
            {
              // array_push($append_string_id, "mock_test_tbl".$key.".".$key." IN (".$value.")"); 
			   array_push($append_string_id, "mock_test_tbl.".$key." IN (".$value.")"); // Changes added by Manjunath
            }
        }
        if($new_input){// Changes added by Manjunath
			array_push($append_string_id, "mock_test_tbl.Email LIKE '%".$new_input."%'");
		}
        if($Final_date!=0)
        {
